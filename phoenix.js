...
// TO DEBUG & LOG run the following in the terminal: log stream --process Phoenix
...
function closeActiveWindow({key, modifiers}) {
...
function getCurrentActiveScreen() {
  ...
  const focused_window = Window.focused().screen().identifier()
  return screens.find(screen => screen.identifier() == focused_window)
}
 ...
 */
function setAppPosition(app, relativeFrame, space, focused_screen) {
  ...
  if (space.screens().length > 1) {
    ...
    const activeScreen = focused_screen
...
async function startApp(appName) {
  ...
  if (app !== undefined) {
    ...
    if (app.windows().length === 0 && app !== 'finder' && appName !== 'Microsoft Edge') {
    ...
    if (appName == 'finder') {
      await osascript('do shell script "open ~/"')
    } else if (appName == 'Microsoft Edge') {
      await osascript(`do shell script "open -na '${appName}' --args --new-window"`)
    } else {
